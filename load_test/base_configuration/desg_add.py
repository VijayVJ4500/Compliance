...
class desgadd(IpssTestCases):
    def __init__(self, driver):
        self.driver = driver

    def load_test_concurrent_users(self, num_requests=100, num_threads=10):
        """
        Perform a load test for creating sequence configurations concurrently.

        Args:
            num_requests (int): Total number of requests to send.
            num_threads (int): Number of concurrent threads.
        """
        url = "https://ipssapi.techgenzi.com/department_hrm/designation/394/"

        payload = {
    "desg_name": "CUTTING OPERATOR",
    "deptmastid": 394,
