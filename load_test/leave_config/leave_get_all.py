...
class Leaveget(IpssTestCases):
    def __init__(self, driver):
        self.driver = driver

    def load_test_concurrent_get_requests(self, num_requests=100, num_threads=10):
        """
        Perform a load test for fetching leave configurations concurrently.

        Args:
            num_requests (int): Total number of requests to send.
            num_threads (int): Number of concurrent threads.
        """
        url = "https://ipssapi.techgenzi.com/leave_configuration_hrm/"  # Endpoint for getting leave configurations
