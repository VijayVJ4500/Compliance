...
class weekoffassign(IpssTestCases):
    def __init__(self, driver):
        self.driver = driver

    def load_test_concurrent_patch_requests(self, num_requests=100, num_threads=10):
        """
        Perform a load test for updatin
        g shift configurations concurrently.

        Args:
            num_requests (int): Total number of requests to send.
            num_threads (int): Number of concurrent threads.
        """
        url = "https://ipssapi.techgenzi.com/weekoff_configure/weekoff_assign_mast/"

        payload = {"emp_id":[29964,29963],"total_date":["2025-03-15"],"type":"date"}
