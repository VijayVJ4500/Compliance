...
class WFHManagementNegative(BasePage):
    """
    WFH management negative test cases.
    """
    def __init__(self, driver):
        super().__init__(driver)
        self.logger = get_logger("WFHManagementNegative")
        self.wait = WebDriverWait(driver, 10)
        self.screenshot_dir = "screenshots/WFH_management_negative"
    ...
    def test_invalid_date(self):
        """Test applying WFH with invalid date format"""
        try:
            ...
            self.enter_text(Locators.EP_WFH_DATE, "15-11-72656")
            time.sleep(1)
            self.enter_text(Locators.EP_WFH_REASON, "Test invalid date")
            time.sleep(1)
            self.click(Locators.EP_WFH_APPLY_BT)
            time.sleep(1)
            # Check for success message
            if self.check_snackbar_message("WFH Applied successfully!"):
                self.logger.error("WFH applied successfully with invalid date")
                self.take_action_screenshot("WFH_approval_success")
                return False
            else:
                self.logger.info("WFH is not Applied  - Success message not shown")
                self.take_action_screenshot("WFH_Apply_failed")
                return False
        except Exception as e:
    ...
    def test_empty_reason(self):
        """Test applying WFH without reason"""
        try:
            self.click(Locators.EP_WFH_SELF)
            time.sleep(1)
            self.click(Locators.EP_WFH_SINGLE_DAY_RADIO)
            ...
            self.enter_text(Locators.EP_WFH_DATE, "10-03-2025")
            time.sleep(1)
            # Don't enter reason
            ...
            self.click(Locators.EP_WFH_APPLY_BT)
             # Check for success message
            if self.check_snackbar_message("WFH Applied successfully!"):
                self.logger.error("WFH applied with empty reason field")
    ...
    def test_past_date(self):
        """Test applying WFH for past date month"""
        try:
            self.click(Locators.EP_WFH_SELF)
            time.sleep(1)
            self.click(Locators.EP_WFH_SINGLE_DAY_RADIO)
            ...
            yesterday = (datetime.now() - timedelta(days=12)).strftime("%d-%m-%Y")
            self.enter_text(Locators.EP_WFH_DATE, yesterday)
            time.sleep(1)
            self.enter_text(Locators.EP_WFH_REASON, "Test past date")
            time.sleep(1)
            
            self.click(Locators.EP_WFH_APPLY_BT)
# Check for success message
            if self.check_snackbar_message("WFH Applied successfully!"):
                self.logger.error("WFH applied with past month field")
                self.take_action_screenshot("WFH_approval_case_failed")
                return False
            else:
                self.logger.info("WFH with Past Month not applied - Success message not shown")
                self.take_action_screenshot("WFH_approvalcase_success")
                return False
        except Exception as e:
            self.logger.error(f"WFH with Past Month test failed: {str(e)}")
            raise
    ...
    def test_reject_without_remarks(self):
        """Test rejecting WFH without providing remarks"""
        try:
