...
class attritiondelete(IpssTestCases):
    def __init__(self, driver):
        self.driver = driver

    def load_test_concurrent_delete_requests(self, num_requests=100, num_threads=10):
        """
        Perform a load test for deleting sequence configurations concurrently.

        Args:
            num_requests (int): Total number of requests to send.
            num_threads (int): Number of concurrent threads.
        """
        url = "https://ipssapi.techgenzi.com/resign/attri_config_hrm/43"  # Example URL to delete a specific configuration
