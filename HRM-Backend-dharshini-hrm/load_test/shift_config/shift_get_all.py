...
class shiftget(IpssTestCases):
    def __init__(self, driver):
        self.driver = driver

    def load_test_concurrent_get_requests(self, num_requests=100, num_threads=10):
        """
        Perform a load test for fetching shift configurations concurrently.

        Args:
            num_requests (int): Total number of requests to send.
            num_threads (int): Number of concurrent threads.
        """
        url = "https://ipssapi.techgenzi.com/shift_configure/shift_mast_hrm/"  # Endpoint for getting shift configurations
