...
class Leaveedit(IpssTestCases):
    def __init__(self, driver):
        self.driver = driver

    def load_test_concurrent_patch_requests(self, num_requests=100, num_threads=10):
        """
        Perform a load test for updating leave configurations concurrently.

        Args:
            num_requests (int): Total number of requests to send.
            num_threads (int): Number of concurrent threads.
        """
        url = "https://ipssapi.techgenzi.com/leave_configuration_hrm/814"

        payload = {
            "leavetype_name": "function leave",
            "description": None,
            "loss_of_pay": False,
            "abbreviation": "AB",
            "types": "Leave",
